...
\begin{proof}
我们仍然只考虑$\gamma$是光滑的情形。通过考虑$\gamma$的实部和虚部, 把证明归结为对于$\gamma([a,b]) \subset \mathbb{R}$的情形.设$\epsilon > 0$,且可取$\delta>0$,使得当$P = \{a=t_0<t_1<\cdots<t_n=b\}$,满足$\|P\| < \delta$时
\begin{gather}
\left|\int_{a}^{b}{fd\gamma} - \sum_{k=1}^{n}{f(\tau_k)[\gamma(t_k) - \gamma(t_{k-1})]}\right| < \epsilon / 2,\label{equ002040110}\\
\left|\int_{a}^{b}{f(t)\gamma'(t)dt} - \sum_{k=1}^{m}{f(\tau_k)\gamma'(\tau_k)(t_k - t_{k-1})}\right| < \epsilon / 2.\label{equ002040111}
\end{gather}
对于$[t_{k-1}, t_k]$上的任意$\tau_k$成立。如果我们应用导数的中值定理，得到在$[t_{k-1}, t_k]$内存在$\tau_k$，使得$\gamma'(\tau_k) = [\gamma(t_k) - \gamma(t_{k-1})](t_k - t_(k-1))^{-1}$（注意：应用中值定理时，$\gamma$必需是实的）。于是
\[
\sum_{k=1}^{n}{f(\tau_k)[\gamma(t_k) - \gamma(t_{k-1})]} = \sum_{k=1}^{n}{f(\tau_k)\gamma'(\tau_k)(t_k - t_{k-1})}.
\]
联系到不等式（\ref{equ002040110}）和（\ref{equ002040111}）就得到
\[
\left|\int_{a}^{b}{fd\gamma} - \int_{a}^{b}{f(t)\gamma'(t)dt}\right| < \epsilon.
\]
由于$\epsilon$是任意的，所以定理得证。
\end{proof}

我们已经定义路径为一个连续函数$\gamma:[a, b] \to \mathbb{C}$.如果$\gamma$是一条路径，那么集合$\gamma(t):a \le t \le b$称为$\gamma$的迹(trace),用$\{\gamma\}$表示。应当注意，路径的迹总是紧集。当$\gamma$是有界变差函数时，$\gamma$是可求长路径。如果$P$是$[a,b]$的一个分割，那么$v(\gamma;P)$正好就是连接$\gamma$的迹上的分点的线段的长度的和。说$\gamma$是可求长的就是说$\gamma$有有穷长度，且它的长度是$V(\gamma)$. 特别地，如果$\gamma$是分段光滑的，那么$\gamma$是可求长的，它的长度等于$\int_{a}^{b}{|\gamma'(t)|dt}$.

如果$\gamma:[a, b] \to \mathbb{C}$是可求长路径，且$\{\gamma\} \subset E \subset \mathbb{C}$，$f: E \to \mathbb{C}$是连续函数，那么$f \circ \gamma$是$[a,b]$上的连续函数。记住这点，有助于理解下面的定义。
\begin{definition}{}{def002040112}
如果$\gamma:[a, b] \to \mathbb{C}$是可求长路径，$f$是定义在$\gamma$的迹上的连续函数，那么$f$沿$\gamma$的（线）积分是
\[
...
作为例子，我们将$\gamma:[a, b] \to \mathbb{C}$取为$\gamma(t) = e^{it}$, 对于$z \neq 0$, 定义$f(z) = \frac{1}{z}$。$\gamma$是光滑的，所以由定理\ref{thm:thm002040109}，
\[
\int_{\gamma}{\frac{1}{z}dz} = \int_{0}^{2\pi}{e^{-it}(ie^{it})dt} = 2\pi{}i.
\]

利用$\gamma$的同一定义，并设$m \ge 0$是任意整数，得到
\[
\begin{aligned}
\int_{\gamma}{z^mdz} &= \int_{0}^{2\pi}{e^{imt}(ie^{it})dt} = \int_{0}^{2\pi}{\exp(i(m+1))dt}\\
&=\int_{0}^{2\pi}{\cos(m+1)tdt} + i\int_{0}^{2\pi}{\sin(m+1)tdt} = 0.
\end{aligned}
\]

现在设$a, b \in \mathbb{C}$, 令$\gamma(t) = tb + (1-t)a$, $0 \le t \le 1$,那么$\gamma'(t) = b-a$,利用微积分学的基本定理，我们便得到
\[
\int_{\gamma}{z^ndz} = (b-a)\int_{0}^{1}{[tb + (1-t)a]^ndt} = \frac{1}{n+1}(b^{n+1} - a^{n+1}),
\]
其中$n \ge 0$.在习题中将举出更多的例子。但现在我们将证明某个“不变性”的结果，这个结果除了可用于计算外，也构成了我们曲线定义的基础。

如果$\gamma:[a, b] \to \mathbb{C}$是可求长路径，$\varphi:[c, d] \to [a, b]$是连续非减函数，它的像是整个$[a,b]$（即$\varphi(c)=a$, $\varphi(d)=b$），则$\gamma\circ\varphi: [c, d] \to \mathbb{c}$是一条路径，它与$\gamma$有相同的迹。而且$\gamma\circ\varphi$是可求长的，因为如果$c=a_0<s_1<\cdots<s_n=d$,那么$a = \varphi(s_0) \le \varphi(s_1) \le \cdots \le \varphi(s_n) = b$是$[a,b]$的一个分割，因此
\[
\sum_{k=1}^{n}{|\gamma(\varphi(s_k)) - \gamma(\varphi(s_{k-1}))|} \le V(\gamma,)
\]
所以$V(\gamma\circ\varphi) \le V(\gamma)<\infty$,如果$f$是$\{\gamma\} = \{\gamma\circ\varphi\}$上的连续函数，那么$\int_{\gamma\circ\varphi}{f}$也是有定义的。

\begin{proposition}{}{prop002040113}
如果$\gamma:[a,b] \to \mathbb{C}$是可求长路径，$\varphi:[c, d] \to [a,b]$是连续非减函数，且$\varphi(c)=a$, $\varphi(d)=b$, 则对于$\{\gamma\}$上的任意一个连续函数$f$,有
\[
...
\begin{proof}
设$\epsilon > 0$，选取$\delta_1 > 0$,使得对于满足$(s_k - s_{k-1})< \delta_1$的$[c, d]$的分割$\{s_0<s_1<\cdots<s_n\}$及$s_{k-1} \le \sigma_k \le s_k$,有
\begin{gather}\label{equ002040114}
\left|\int_{\gamma\circ\varphi}{f} - \sum_{k=1}^{n}{f(\gamma\circ\varphi(\sigma_k))[\gamma\circ\varphi(s_k) - \gamma\circ\varphi(s_{k-1})]}\right| < \epsilon/2.
\end{gather}
类似地，选取$\delta_2>0$，使得对于满足$(t_k - t_{k-1})<\delta_2$的$[a,b]$的分割$\{t_0 < t_1 < \cdots < t_n\}$及$t_{k-1}\le \tau_k \le t_k$，有
\begin{gather}\label{equ002040115}
\left|\int_{\gamma}{f} - \sum_{k=1}^{n}{f(\gamma(\tau_k))[\gamma(t_k) - \gamma(t_{k-1})]}\right| < \epsilon/2.
\end{gather}
但是$\varphi$在$[c, d]$上是一致连续的，因此存在$\delta > 0$,$\delta<\delta_1$,使得当$|s-s'|<\delta$时,就有$|\varphi(s)-\varphi(s')|<\delta_2$,所以，如果$\{s_0 < s_1 < \cdots<s_n\}$是$[c,d]$的一个分割，满足$(s_k - s_{k-1})<\delta<\delta_1$,令$t_k = \varphi(s_k)$，那么$\{t_0<t_1<\cdots<t_n\}$是$[a,b]$的一个分割，满足$(t_k - t_{k-1})<\delta_2$.如果$s_{k-1} \le \sigma_k \le s_k$, 令$\tau_k = \varphi(\sigma_k)$,那么（\ref{equ002040114}）和（\ref{equ002040115}）都成立，而且这两个差式的第二项是相同的，由此推出
\[
...
我们希望在可求长路径的集合上定义一个等价关系，使得等价类中的每一条路径有相同的迹，并且在这个迹上的连续函数的线积分，对于这个类中的每一条路径来说都是相同的。如果对于某一个像上面那样的\footnote{所谓“像上面那样的$\varphi$”是指不减的$\varphi$---校者注}$\varphi$，$\sigma = \gamma\circ\varphi$，我们似乎就应当定义$\sigma$和$\gamma$是等价的，但这并不是等价关系！

\begin{definition}{}{def002040116}
设$\sigma:[c, d]\to \mathbb{C}$和$\gamma:[a, b] \to\mathbb{C}$都是可求长路径
。称路径$\sigma$和$\gamma$等价，如果存在连续、严格增函数$\varphi:[c, d]\to[a,b]$，且$\varphi(c)=a$, $\varphi(d)=b$,使得$\sigma = \gamma\circ\varphi$.我们称函数$\varphi$为一个参数变换。
\end{definition}
...
今后，曲线和它的等价类中的路径我们将不加区别。假如“设$\gamma$是按照反时针方向通过一次的单位圆周”就表示一条曲线。读者应当相信，今后所叙述的关于曲线的结果，与具体路径的取法无关。

设$\gamma:[a,b]\to\mathbb{C}$是一个可求长路径，且对于$a \le t \le b$, 令$|\gamma|(t)$为$V(\gamma;[a,t])$,即
\[
|\gamma|(t) = \sup\left\{\sum_{k=1}^{n}{|\gamma(t_k) - \gamma(t_{k-1})|}:\{t_0<t_1<\cdots<t_n\}\text{是}[a,t]\text{的分割}\right\}.
\]
显然，$|\gamma|(t)$是增函数，所以$|\gamma|:[a, b]\to\mathbb{C}$是有界变差函数。如果$f$在$\{\gamma\}$上连续，定义
\[
\int_{\gamma}{f|dz|} = \int_{a}^{b}{f(\gamma(t))d|\gamma|(t)}.
\]
如果$\gamma$是可求长曲线，那么用$-\gamma$表示由$(-\gamma)(t)=\gamma(-t)$定义的曲线，$-b \le t \le -a$.其另一个记号是$\gamma^{-1}$.此外，如果$c \in \mathbb{C}$,$\gamma+c$表示由$(\gamma+c)(t)=\gamma(t)+c$定义的曲线。下面的命题给出线积分的许多基本性质。
\begin{proposition}{}{prop002040117}
设$\gamma$是可求长曲线，$f$是$\{\gamma\}$上的连续函数，则
\begin{enumerate}
\item[(a)] $\int_{\gamma}{f} = -\int_{-\gamma}{f}$;
\item[(b)] $|\int_{\gamma}{f}| \le \int_{\gamma}{|f||dz|} \le V(\gamma)\sup[|f(z)|:z \in \{\gamma\}]$;
\item[(c)] 如果$c \in \mathbb{C}$,那么$\int_{\gamma}{f(z)dz} = \int_{\gamma+c}{f(z)dz}$.
\end{enumerate}
\end{proposition}
...
\begin{theorem}{}{thm002040118}
设$G$是$\mathbb{C}$中的开集，$\gamma$是$G$内的可求长路径，其起点和终点分别是$\alpha$和$\beta$.如果$f:G \to\mathbb{C}$是连续函数，且具有原函数$F:G\to\mathbb{C}$.则
\[
...
在这个定理的证明中，需要下面有用的事实。
\begin{lemma}{}{lemma002040119}
如果$G$是$\mathbb{C}$中的开集，$\gamma:[a,b]\to G$是可求长路径，$f:G\to\mathbb{C}$是连续的，则对于每个$\epsilon>0$,存在$G$内的折线$\Gamma$,使得$\Gamma(a)=\gamma(a)$, $\Gamma(b)=\gamma(b)$,并且$\left|\int_{\gamma}{f}-\int_{\Gamma}{f}\right|<\epsilon$.
\end{lemma}

\begin{proof}
\textbf{情形I}\quad 设$G$是一个开圆。由于$\{\gamma\}$是紧集，$d = \dist(\{\gamma\}, \partial{G})>0$.由此推出，如果$G=B(c;r)$,那么$\{\gamma\} \subset B(c;\rho)$,其中$\rho = r - \frac{1}{2}d$.过渡到这个较小的圆的理由是，$f$在$\bar{B}(c;\rho) \subset G$上是一致连续的，因此，不失一般性，可以假定$f$在$G$内是一致连续的。选取$\delta>0$,使得当$|z-w|<\delta$时,$|f(z)-f(w)|<\epsilon$.如果$\gamma:[a, b]\to\mathbb{C}$，那么$\gamma$是一致连续的。于是存在$[a,b]$的分割$\{t_0<t_1<\cdots<t_n\}$使得当$t_{k-1}\le s,t\le t_k$时，
\begin{gather}\label{equ002040119}
|\gamma(s) - \gamma(t)| < \delta.
\end{gather}
并且对于$t_{k-1} \le \tau_k \le t_k$,有
\begin{gather}\label{equ002040120}
\left|\int_{\gamma}{f} - \sum_{k=1}^{n}{f(\gamma(\tau_k))[\gamma(t_k) - \gamma(t_{k-1})]}\right|<\epsilon.
\end{gather}
我们定义$\Gamma:[a,b]\to\mathbb{C}$为
\begin{gather*}
\Gamma(t) = \frac{1}{t_k - t_{k-1}}[(t_k-t)\gamma(t_{k-1}) + (t - t_{k-1})\gamma(t_k)],\\
t_{k-1} \le t \le t_{k}.
\end{gather*}
所以在$[t_{k-1}, t_k]$上，$\Gamma(t)$是从$\gamma(t_{k-1})$到$\gamma(t_k)$的线段，即$\Gamma$是$G$内的折线路径。由(\ref{equ002040119})得到
\begin{gather}\label{equ002040121}
|\Gamma(t) - \gamma(\tau_k)|<\delta, t_{k-1} \le t \le t_k.
\end{gather}

由于$\int_{\Gamma}{f} = \int_{a}^{b}{f[\Gamma(t)]\Gamma'(t)dt}$, 所以
\[
\int_{\Gamma}{f} = \sum_{k=1}^{n}{\frac{\gamma(t_k) - \gamma(t_{k-1})}{t_k - t_{k-1}}\int_{t_{k-1}}^{t_k}{f(\Gamma(t))dt}}.
\]
利用（\ref{equ002040120}），我们得到
\[
\begin{aligned}
\left|\int_{\gamma}{f} - \int_{\Gamma}{f}\right| &\le \epsilon + \left|\sum_{k=1}^{n}{f(\gamma(\tau_k))}\right.\\
&\qquad\left.\cdot[\gamma(t_k) - \gamma(t_{k-1})] - \int_{\Gamma}{f}\right|\\
&\le\epsilon + \sum_{k=1}^{n}{|\gamma(t_k) - \gamma(t_{k-1})|(t_k - t_{k-1})^{-1}}\\
&\qquad\cdot\int_{t_{k-1}}^{t_k}{|f(\Gamma(t)) - f(\gamma(\tau_k))|dt}.
\end{aligned}
\]
应用（\ref{equ002040121}）于被积函数，得到
\[
\left|\int_{\gamma}{f} - \int_{\Gamma}{f}\right| \le \epsilon + \epsilon\sum_{k=1}^{n}{|\gamma(t_k) - \gamma(t_{k-1})|} \le \epsilon(1 + V(\gamma)).
\]
情形I得证。

\textbf{情形II}\quad $G$是任意的。由于$\{\gamma\}$是紧集，所以存在数$r>0$,$0<r<\dist(\{\gamma\}, \partial{G})$.选取$\delta>0$，使得当$|s-t|<\delta$时, $|\gamma(s) - \gamma(t)|<r$.如果$P=\{t_0<t_1<\cdots<t_n\}$是$[a,b]$的一个分割，且$\|P\| < \delta$，那么对于$t_{k-1} \le t \le t_k$,有$|\gamma(t) - \gamma(t_{k-1})|<r$.即，如果$\gamma_k:[t_{k-1}, t_k]\to\mathbb{C}$定义为$\gamma_k(t)=\gamma(t)$,那么$\{\gamma_k\} \subset B(\gamma(t_{k-1});r)$, $1 \le k \le n$.由情形I，存在一条折线路径$\Gamma_k:[t_{k-1}, t_k]\to B(\gamma(t_{k-1});r)$使得$\Gamma(t_{k-1}) = \gamma(t_{k-1})$, $\Gamma(t_k) = \gamma(t_k)$，且$\left|\int_{\gamma_k}{f} - \int_{\Gamma_k}{f}\right|<\epsilon/n$.在$[t_{k-1}, t_k]$上让$\Gamma(t) = \Gamma_k(t)$,那么$\Gamma$具有所要求的性质。
\end{proof}

\textbf{定理\ref{thm:thm002040118}的证明} \quad \textbf{情形I}\quad $\gamma:[a, b] \to \mathbb{C}$是分段光滑的。这时由微积分学的基本定理，
\[
\begin{aligned}
\int_{\gamma}{f} &= \int_{a}^{b}{f(\gamma(t))\gamma'(t)dt} = \int_{a}^{b}{F'(\gamma(t))\gamma'(t)dt}\\
&=\int_{a}^{b}{(F \circ \gamma)'(t)dt} = F(\gamma(b)) - F(\gamma(a)) \\
&= F(\beta) - F(\alpha).
\end{aligned}
\]

\textbf{情形II}\quad 一般情形，若$\epsilon > 0$, 由引理\ref{lem:lemma002040119}, 存在从$\alpha$到$\beta$的折线路径$\Gamma$, 使得$\left|\int_{\gamma}{f} - \int_{\Gamma}{f}\right| < \epsilon$, 但是$\Gamma$是分段光滑的，所以由情形I, $\int_{\Gamma}{f} = F(\beta) - F(\alpha)$. 因此$\left|\int_{\gamma}{f} - [F(\beta) - F(\alpha)]\right| < \epsilon$;由于$\epsilon$是任意的。我们所要求的等式得证。

在定理\ref{thm:thm002040118}的证明中，利用引理\ref{lem:lemma002040119}，从分段光滑的情形过渡到可求长的情形，这种方法在关于线积分许多结果的证明中是有代表性的。今后我们还会看到引理\ref{lem:lemma002040119}的应用。

称一条曲线$\gamma:[a, b] \to \mathbb{C}$是闭的，如果$\gamma(a) = \gamma(b)$.

\begin{corollary}{}{coro002040122}
设$G$, $\gamma$和$f$满足定理\ref{thm:thm002040118}中的假定，如果$\gamma$是闭曲线，则
\[
\int_{\gamma}{f} = 0.
\]
\end{corollary}

微积分学的基本定理告诉我们，每一个连续函数必有原函数。对于复变函数来说，情况远非如此，例如，设$f(z) = x^2 + y^2$, 如果$F$是$f$的原函数。现在由Cauchy-Riemann方程，得到
\[
\frac{\partial{U}}{\partial{x}} = \frac{\partial{V}}{\partial{y}} = x^2+y^2, \frac{\partial{U}}{\partial{y}} = \frac{\partial{V}}{\partial{x}} = 0.
\]
但是$\frac{\partial{U}}{\partial{y}} = 0$蕴含着$U(x, y) = u(x)$, $u(x)$是某一个可微的函数，所以$x^2 + y^2 = \frac{\partial{U}}{\partial{x}} = u'(x)$，这是一个明显的矛盾。应用定理\ref{thm:thm002040118}也可以看出$|z|^2$没有原函数（见习题\ref{exer002040108}）。


\begin{problemset}
\item\label{exer002040101}设$\gamma:[a, b] \to \mathbb{R}$是非减的, 证明：$\gamma$是有界变差函数，且$V(\gamma) = \gamma(b) - \gamma(a)$.

\item 证明命题\ref{pro:prop002040102}.

\item 证明命题\ref{pro:prop002040107}.

\item 证明命题\ref{pro:prop002040108}（利用归纳法）.

\item 设$\gamma(t) = \exp{((-1 + i)t^{-1})}$, $0 < t < 1$,且$\gamma(0) = 0$,证明：$\gamma$是可求长路径，并求出$V(\gamma)$，作出$\gamma$的迹的略图。

\item 证明：如果$\gamma:[a, b] \to \mathbb{C}$是Lipschitz函数，则$\gamma$是有界变差函数。

\item 设$\gamma:[a, b] \to \mathbb{C}$定义为$\gamma(t) = t + it\sin{\frac{1}{t}}$，当$t \neq 0$时. $\gamma(0) = 0$.证明$\gamma$是一条路径，但不是可求长的，作出这一条路径。

\item\label{exer002040108}设$\gamma$和$\sigma$是两条折线$[1, i]$和$[1, 1+i, i]$. 试将$\gamma$和$\sigma$表示为路径，并计算$\int_{\gamma}{f}$和$\int_{\sigma}{f}$,其中$f(z) = |z|^2$.
